...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/certs/formWidget/>):
   1. Run `npm i`
...
- **certs.form.createError** (fired when an error occurs during certs creation)
- **certs.form.create** (fired when the certs is successfully created)
- **certs.form.updateError** (fired when an error occurs during certs update)
- **certs.form.update** (fired when the certs is successfully updated)
