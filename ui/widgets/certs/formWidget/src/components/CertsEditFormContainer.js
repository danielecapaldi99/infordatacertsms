...
import { apiCertsGet, apiCertsDelete, apiCertsPut } from 'api/certs';
import Notification from 'components/common/Notification';
import CertsForm from 'components/CertsForm';

class CertsEditFormContainer extends PureComponent {
  constructor(props) {
    super(props);
  ...
  async fetchData() {
    ...
    if (authenticated && id) {
      try {
        const certs = await apiCertsGet(serviceUrl, id);
        this.setState(() => ({
          certs,
        }));
      } catch (err) {
  ...
  async handleSubmit(certs) {
    ...
    if (authenticated) {
      try {
        const updatedCerts = await apiCertsPut(serviceUrl, certs);
  ...
  async handleDelete(certs) {
    const { t, onDelete, keycloak, serviceUrl } = this.props;
    const authenticated = keycloak.initialized && keycloak.authenticated;

    if (authenticated) {
      try {
        await apiCertsDelete(serviceUrl, certs.id);
        onDelete(certs);
        this.setState({
          certs: null,
  ...
  render() {
    ...
    const { notificationMessage, notificationStatus, certs } = this.state;

    let form;
    if (typeof certs === 'undefined') {
      form = t('entities.certs.notFound');
    } else if (certs === null) {
      form = t('entities.certs.deleted');
    } else {
      form = (
        <CertsForm
          certs={certs}
...
CertsEditFormContainer.propTypes = {
...
CertsEditFormContainer.defaultProps = {
  onCancelEditing: () => {},
  onDelete: () => {},
  onUpdate: () => {},
  onError: () => {},
...
export default withKeycloak(withTranslation()(CertsEditFormContainer));
